...
class kontose {
    ...
    static getBank(clearing: number) {
    ...
    static* getIbanBanks(ibanId: number) : IterableIterator<bankItem> {
        for (let bank of this.banks) {
            if (bank.i == ibanId)
                yield bank;
        }
    ...
    static valid(value: string) {
        ...
        return acclen === 7 && (
            this.mod11check(nums) ||
            this.mod11check(nums.substring(1)));
    }
}

if (kontose.getBank(3300)!.c != 1) console.error("fail test");
if (kontose.getBank(9549)!.c != 3) console.error("fail test");
if (kontose.getBank(1000) != null) console.error("fail test");
if (!kontose.valid("3300, 1010101010")) console.error("fail test");
if (kontose.valid("3300, 1010101011")) console.error("fail test");
if (kontose.valid("81509, 1")) console.error("fail test");
